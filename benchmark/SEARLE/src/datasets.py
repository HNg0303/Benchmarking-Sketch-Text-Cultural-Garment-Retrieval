...
class AoDaiDataset(Dataset):
    """
    AoDai dataset class for PyTorch.
    The dataset yields a dict with keys:
        - ['reference_image', 'reference_name', 'target_image', 'target_name', 'relative_captions']
        - when split in ['train', 'val']
        - in 2 modes: 'relative' and 'classic' -> Classic: only yields images while relative yields triplets
    """
    def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'],
                 preprocess: callable, mode: Literal['relative', 'classic', "sketch"] = 'relative'):
        """
        :param dataset_path: path to the AoDai dataset
        ...
        if mode == "relative":
            with open(dataset_path / 'captions' / f'cap.{split}.json') as f:
                self.triplets: List[dict] = json.load(f)
        if mode == "classic":
            self.init_images()
            print(f"Initialize Ao Dai Images")
        print(f"AoDai {split} dataset initialized")
    
    def init_images(self):
        root_image = self.dataset_path / "images"
        for file_path in root_image.iterdir():
            # 1. Open and preprocess the image
            image = self.preprocess(PIL.Image.open(file_path))
            self.image_names.append(file_path.name) 
            self.images.append(image)

    def init_sketches(self):
        root_sketch = self.dataset_path / "sketches"
        for file_name in root_sketch.iterdir():
            sketch_path = root_sketch / f"{file_name}"
            sketch = self.preprocess(PIL.Image.open(sketch_path))
            self.sketches.append(sketch)
            self.sketch_names.append(str(file_name))
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == "relative":
            if self.split == "train":
                relative_captions = self.triplets[index]['caption']
                sketch_name = self.triplets[index]['sketch']
                sketch_image_path = self.dataset_path / 'sketches' / f"{sketch_name}"
                sketch_image = self.preprocess(PIL.Image.open(sketch_image_path))
                target_name = self.triplets[index]['target']
                target_image_path = self.dataset_path / 'images' / f"{target_name}"
                target_image = self.preprocess(PIL.Image.open(target_image_path))

                return {
                    'reference_image': sketch_image,
                    'reference_name': sketch_name,
                ...
                sketch_name = self.triplets[index]['sketch']
                sketch_path =  self.dataset_path / "sketches" / f"{sketch_name}"
                sketch_image = self.preprocess(PIL.Image.open(sketch_path))
                caption = self.triplets[index]['caption']
                target_path_lists = self.triplets[index]['images'] # List of 3 target paths.
                return {
                    "reference_image": sketch_image,
                    "reference_name": sketch_name,
                    "relative_caption": caption,
                    "target_images": target_path_lists
                }
