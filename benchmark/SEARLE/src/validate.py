...
from datasets import FashionIQDataset, CIRRDataset, CIRCODataset, AoDaiDataset
...
@torch.no_grad()
def aodai_generate_val_predictions(clip_model: CLIP, relative_val_dataset: Dataset, ref_names_list: List[str],
                                 pseudo_tokens: torch.Tensor) -> Tuple[torch.Tensor, List[str]]:
    """
    Generate features prediction for validation set of the Ao Dai dataset.
    """
    ...
    for batch in tqdm(relative_val_loader):
        ...
        target_names = batch['target_images']
        ...
        tokenized_input_captions = clip.tokenize(input_captions, context_length=77, truncate = True).to(device)
        print(f"Text shape: {tokenized_input_captions.shape}")
...
@torch.no_grad()
def aodai_compute_metric(relative_val_dataset: Dataset, clip_model: CLIP, index_features: torch.Tensor,
                            index_names: List[str], ref_names_list: List[str], pseudo_tokens: torch.Tensor) \
                            -> Dict[str, float]:
    """
    Computes metrics for the Ao Dai dataset by generating query features 
    and matching them against the gallery.
    """
    # 1. Generate composed query features and their corresponding target names (ground truth)
    # Note: relative_val_dataset should return 'answer_paths' or similar for ground truth
    predicted_features, target_names_list = aodai_generate_val_predictions(
        clip_model, relative_val_dataset, ref_names_list, pseudo_tokens
    )
    
    # 2. Extract answer_names from the dataset for the ground truth matrix
    # Based on our previous discussion, each query has multiple correct answers.
    # We collect them here to ensure order matches predicted_features.
    answer_names = []
    for i in range(len(relative_val_dataset)):
        # dataset[i] returns (reference_img, target_img, captions, list_of_3_answers)
        # We need that list of 3 answers
        answers = relative_val_dataset[i]["target_images"]
        answer_names.append(answers)
    
    print(f"Sample target names: {answer_names[0]}")

    # 3. Compute the retrieval metrics
    # index_features: (num_gallery, dim), predicted_features: (num_queries, dim)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Images Features: {index_features.shape}")
    print(f"Predicted Features: {predicted_features.shape}")
    print(f"Number of ground truth: {len(index_names)}")
    print(f"Number of ground truth (val): {len(answer_names)}")
    metrics = get_metrics_aodai(
        image_features=index_features.to(device), 
...
def get_metrics_aodai(image_features, ref_features, target_names, answer_names):
    ...
    return {
        ...
        "R@50": recall[:, 49].mean() if top_k >= 50 else None,
        "mAP": map_,
        "P@10": np.mean(precision[:, 9]) if top_k >= 10 else None,
        "MRR": mrr
    }

def aodai_retrieval(dataset_path: str, clip_model_name: str, ref_names_list: List[str],
    ...
    classic_val_dataset = AoDaiDataset(dataset_path = dataset_path, split = 'val', preprocess = preprocess, mode = 'classic')
    index_features, index_names = extract_image_features(classic_val_dataset, clip_model)
    print(f"Sample image name: {index_names[0]}")

    # Define the relative dataset
    relative_val_dataset = AoDaiDataset(dataset_path, 'val', preprocess, 'relative')

    return aodai_compute_metric(relative_val_dataset, clip_model, index_features, index_names, ref_names_list,
                                   pseudo_tokens)
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True, choices=['cirr', 'fashioniq', 'circo', 'aodai'],
                        help="Dataset to use")
    ...
    if args.eval_type == 'oti':
        ...
        if args.dataset.lower() == 'fashioniq':
            ...
            relative_val_dataset = AoDaiDataset(dataset_path = args.dataset_path,  split = 'val', preprocess = preprocess, mode = "relative")
        else:
            raise ValueError("Dataset not supported")

        clip_model = clip_model.float().to(device)
        pseudo_tokens, ref_names_list = extract_pseudo_tokens_with_phi(clip_model, phi, relative_val_dataset) # Return pseudo tokens of sketches and list of sketches names.
        pseudo_tokens = pseudo_tokens.to(device)
        print(f"Pseudo Token Shape: {pseudo_tokens.shape}")
    else:
    ...
    if args.dataset.lower() == 'fashioniq':
        ...
        aodai_metrics = aodai_retrieval(args.dataset_path, clip_model_name, ref_names_list, pseudo_tokens, preprocess)

        for k, v in aodai_metrics.items():
            print(f"{k} = {v:.2f}")
        with open("../ao_dai_metrics.json", "w") as f:
            import json
            json.dump(aodai_metrics, f, indent = 4)
