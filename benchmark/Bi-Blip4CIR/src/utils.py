...
def extract_index_features(dataset: Union[CIRRDataset, FashionIQDataset, AoDaiDataset], blip_model: nn.Module) -> \
        Tuple[torch.tensor, List[str]]:
    """
    Extract FashionIQ or CIRR index features
    :param dataset: FashionIQ or CIRR dataset in 'classic' mode
...
#     """
    ...
    if isinstance(dataset, CIRRDataset):
        ...
        print(f"[{datetime.now()}] extracting fashionIQ {dataset.dress_types} - {dataset.split} index features")
    elif isinstance(dataset, AoDaiDataset):
        print(f"[{datetime.now()}] extracting AoDai - {dataset.split} index features")
    
    # original
    for images, names in tqdm(classic_val_loader):
        images = images.to(device, non_blocking=True)
        with torch.no_grad():
            batch_features = blip_model(images)
            index_features = torch.vstack((index_features, batch_features))
...
def extract_sketch_features(dataset: Union[CIRRDataset, FashionIQDataset, AoDaiDataset], blip_model: nn.Module) -> \
        Tuple[torch.tensor, List[str]]:
    """
    Extract FashionIQ or CIRR index features
    :param dataset: FashionIQ or CIRR or AoDai dataset in 'classic' mode
    :param blip_model: BLIP model
    :return: a tensor of features and a list of images
    """
    # feature_dim = 256
    ...
    #     with torch.no_grad():
    #         sketch_feature = blip_model(sketch_images.to(device))
    #         sketch_features.append(sketch_feature.cpu())
    #         sketch_names.extend([n[0] if isinstance(n, list) else n for n in sketch_name])
    ...
    # original
    for names, images, caption, target in tqdm(classic_val_loader):
        images = images.to(device, non_blocking=True)
        with torch.no_grad():
            batch_features = blip_model(images)
            index_features = torch.vstack((index_features, batch_features))
