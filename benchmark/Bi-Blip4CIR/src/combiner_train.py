...
from utils import collate_fn, update_train_running_results, set_train_bar_description, save_model, \
    extract_index_features, generate_randomized_fiq_caption, device, extract_sketch_features
from validate import compute_cirr_val_metrics, compute_fiq_val_metrics, compute_aodai_val_metrics

def combiner_training_aodai(num_workers: int,
    ...
    """
    Train the Combiner on AoDai dataset keeping frozed the BLIP model
    ...
    image_features_list, image_names_list = extract_index_features(classic_val_dataset, blip_img_encoder)
    sketch_features_list, sketch_names_list = extract_sketch_features(relative_val_dataset, blip_img_encoder)
    ...
    relative_train_dataset = AoDaiDataset('train', 'relative', preprocess)
    ...
    for epoch in range(num_epochs):
        ...
        with experiment.train():
            ...
            for idx, (reference_images, captions, target_images) in enumerate(train_bar):  # Load a batch of triplets
                ...
                input_captions = [caption for caption in captions]
        ...
        if epoch % validation_frequency == 0:
            ...
            with experiment.validate():
                ...
                # })
                results_dict = compute_aodai_val_metrics(relative_val_dataset, blip_text_encoder, sketch_features_list, sketch_names_list, image_features_list, image_names_list, combiner.combine_features)
                results_dict["average_recall"] = (results_dict["R@1"] + results_dict["R@5"] + results_dict["R@10"]) / 3
...
if __name__ == '__main__':
    ...
    if args.dataset.lower() not in ['fashioniq', 'cirr', 'aodai']:
        raise ValueError("Dataset should be either 'CIRR' or 'FashionIQ or AoDai")
