...
def generate_aodai_val_predictions(blip_model, relative_val_dataset,
    ...
    for sketch_name, sketch_image, captions, batch_target_names in tqdm(relative_val_loader):
        ...
        with torch.no_grad():
            text_features = blip_model(text_inputs, 77, device)
...
def compute_aodai_val_metrics(relative_val_dataset, blip_model, index_features, index_names, images_features, images_names, combining_function) -> Tuple[float, float]:
    # 1. Get predictions
    predicted_features, target_names_list = generate_aodai_val_predictions(
        blip_model, relative_val_dataset, combining_function, index_features, index_names
    )
