...
class AoDaiDataset(Dataset):
    """
    AoDai dataset class which manage AoDai data.
    ...
    def __init__(self, split: str, mode:str, preprocess: callable):
        ...
        self.triplet: List[dict] = []
        with open(base_path / 'aodai' / 'captions' / f'cap.{split}.json', encoding='utf-8') as f:
            self.triplet = json.load(f)
        
        if self.mode == "classic":
            self.init_images()
            print(f"Initialize images -> total {len(self.images)}")

        # # get the image names
        # with open(base_path / 'aodai' / 'image_splits' / f'{split}.json') as f:
        #     self.image_names: list = json.load(f)

        print(f"AoDai {split} dataset initialized")

    def init_images(self):
        root_imgs = base_path / 'aodai' / 'images'
        for file_name in root_imgs.iterdir():
            self.image_names.append(file_name.name)
            self.images.append(self.preprocess(PIL.Image.open(file_name)))
    ...
    def __getitem__(self, index):
        try:
            if self.split not in ['test', 'train', 'val']:
                raise ValueError("split should be in ['test', 'train', 'val']")
            
            if self.mode == 'relative':
                if self.split == 'train':
                    sketch_name = self.triplet[index]['sketch']
                    image_name = self.triplet[index]['image']  # in train mode, we have only one image per entry
                    sketch_path = base_path / 'aodai' / 'sketches' / f"{sketch_name}"
                    sketch_image = self.preprocess(PIL.Image.open(sketch_path))
                    image_path = base_path / 'aodai' / 'images' / f"{image_name}"
                    image = self.preprocess(PIL.Image.open(image_path))
                    caption = self.triplet[index]['caption']
                    return sketch_image, caption, image
                ...
                if self.split in ['val', 'test']:
                    ...
                    sketch_path = base_path / 'aodai' / 'sketches' / f"{sketch_name}"
                    sketch_image = self.preprocess(PIL.Image.open(sketch_path))
                    target_images_name = self.triplet[index]['images']
                    return sketch_name, sketch_image, caption, target_images_name
...
if __name__ == "__main__":
    ...
    dataset = AoDaiDataset(split='train', preprocess=preprocess)
    print(f"Dataset length: {len(dataset)}")
    sketch_image, caption, image = dataset[0]
    print(f"Sketch image shape: {sketch_image.shape}, Caption: {caption}, Image shape: {image.shape}")
