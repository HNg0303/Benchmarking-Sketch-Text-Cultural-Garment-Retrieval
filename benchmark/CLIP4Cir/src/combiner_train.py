...
from data_utils import base_path, squarepad_transform, FashionIQDataset, targetpad_transform, CIRRDataset, AoDaiDataset
from combiner import Combiner
from utils import collate_fn, update_train_running_results, set_train_bar_description, save_model, \
    extract_index_features, generate_randomized_fiq_caption, device, extract_aodai_index_features
from validate import compute_aodai_val_metrics, compute_cirr_val_metrics, compute_fiq_val_metrics

base_path = Path(__file__).absolute().parents[1].absolute()

def combiner_training_aodai(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str,
    ...
    # date only get : YYYY-MM-DD
    training_start = datetime.now().strftime("%Y-%m-%d_%H_%M_%S")
    training_path: Path = Path(
        base_path / f"models/combiner_trained_on_aodai_{clip_model_name}_{training_start}")
    ...
    # Define validation dataset and extract the validation index features
    relative_aodai_val_dataset = AoDaiDataset('val', 'relative', preprocess)
    classic_aodai_val_dataset = AoDaiDataset('val', 'classic', preprocess)
    aodai_sketches_features, aodai_sketches_names, aodai_gallery_features, aodai_gallery_names = extract_aodai_index_features(
        classic_aodai_val_dataset, clip_model
    )
    ...
    train_dataset = AoDaiDataset('train', 'relative', preprocess)
    print(f"Number of training samples: {len(train_dataset)}")
    # If this prints 0, the issue is how the JSON is being read.
    ...
    for epoch in range(num_epochs):
        ...
        with experiment.train():
            ...
            for idx, (reference_images, captions, target_images) in enumerate(train_bar):  # Load a batch of triplets
        ...
        if epoch % validation_frequency == 0:
            clip_model = clip_model.float()  # Switch to fp32 for stable validation
            with experiment.validate():
                combiner.eval()
                
                # 1. Compute AoDai Metrics
                # Assuming relative_aodai_dataset is initialized elsewhere in your code
                aodai_recall_at10, aodai_recall_at50 = compute_aodai_val_metrics(
                    relative_aodai_val_dataset, clip_model, aodai_sketches_features, aodai_sketches_names,
                    aodai_gallery_features, aodai_gallery_names, combiner.combine_features
                )

                # 3. Aggregate Results
                results_dict = {
                    'aodai_recall_at10': aodai_recall_at10,
                    'aodai_recall_at50': aodai_recall_at50
                }
                # Define your primary target for "best model" saving
                # Here we use the average of AoDai and FashionIQ recalls
                results_dict['average_recall'] = (aodai_recall_at10 + aodai_recall_at50 / 2)
            ...
            if save_training:
                if save_best and results_dict['average_recall'] > best_avg_recall:
                    ...
                    print(f"New best model saved at epoch {epoch} with Recall: {best_avg_recall:.2f}")
                elif not save_best:
...
if __name__ == '__main__':
    ...
    if args.dataset.lower() not in ['fashioniq', 'cirr', 'aodai']:
        raise ValueError("Dataset should be either 'CIRR', 'FashionIQ', or 'AoDai'")
    ...
    if args.dataset.lower() == 'cirr':
        ...
        combiner_training_aodai(**training_hyper_params)
