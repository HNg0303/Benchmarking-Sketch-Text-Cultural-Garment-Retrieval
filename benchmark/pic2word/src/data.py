...
class AoDai(Dataset):
    def __init__(self, transforms, root, mode):
        # There will be 2 modes: Images and Captions mode.
        self.root = root
        self.root_img = os.path.join(self.root, 'images')
        self.root_sketch = os.path.join(self.root, 'sketches')
        self.sketches = []                 
        self.images = []
        self.captions = []          
        self.transforms = transforms
        ...
        if self.mode == 'imgs':
            self.init_images()
            logging.info("Use {} imgs".format(len(self.images)))
        else:
            self.json_file = os.path.join(self.root, 'captions', 'cap.val.json')
            logging.debug(f'Loading json data from {self.json_file}.')
            self.data = json.load(open(self.json_file, "r", encoding='utf-8'))  
            self.init_data()     
            logging.info("Use {} imgs".format(len(self.images)))

    def init_images(self):
        # Load only target images all for evaluation.
        for file_name in os.listdir(self.root_img):
            if file_name.endswith('.png') or file_name.endswith('.jpg'):
                self.images.append(os.path.join(self.root_img, file_name))

    def init_data(self):
        # Load all data for evaluation. (Sketches + Caption -> List of Target Images)
        for d in self.data:
            sketch_path = os.path.join(self.root_sketch, d['sketch'])
            caption = d['caption']
            target_path_list = [os.path.join(self.root_img, target_path) for target_path in d['images']] # List of 3 target images for each query.
            self.sketches.append(sketch_path)
            self.captions.append(caption)
            self.images.append(target_path_list)

    def __len__(self):
        return len(self.images)

    def return_imgs(self, idx):
        # Return all target images for evaluation if mode = 'Images'
        img_path = str(self.images[idx])
        target_images = self.transforms(Image.open(img_path))
        return target_images, img_path
    def return_all(self, idx):
        # Return sketch image, caption and list of target images if mode = 'Captions'
        sketch_path = str(self.sketches[idx])
        sketch_image = self.transforms(Image.open(sketch_path))
        caption = self.captions[idx]
        text_with_blank = 'a photo of * , {}'.format(caption)
        token_texts = tokenize(text_with_blank, context_length=77, truncate = True)[0]   
        target_path_list = self.images[idx] # List of 3 target images for each query.          
        return sketch_image, token_texts, target_path_list
...
def get_aodai_dataset(args, preprocess_fn, is_train, input_filename = None):
    if input_filename is None:
        input_filename = args.train_data if is_train else args.val_data
    assert input_filename
    path_data = args.train_data
...
def get_dataset_fn(data_path, dataset_type):
    ...
    elif dataset_type == "aodai":
        return get_aodai_dataset       
