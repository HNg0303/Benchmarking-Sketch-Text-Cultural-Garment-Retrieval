...
def evaluate_aodai(model, img2text, args, source_loader, target_loader):
    # source loader is the query set, target loader is the gallery set.
    if not is_master(args):
        return
    model.eval()
    img2text.eval()
    ...
    # Extract all target images features -> Gallery features
    with torch.no_grad():
        for batch in tqdm(target_loader): # Each batch is (Target_images, target_paths)
            target_images, target_paths = batch
            # if args.gpu is not None:
            target_images = target_images.cuda(args.gpu, non_blocking=True)
            image_features = m.encode_image(target_images)
    ...
    with torch.no_grad():
        for batch in tqdm(source_loader): # Each batch is (ref_sketches, tokenized_caption and list of target_paths)
            ref_sketches, target_caption, answer_paths_list = batch
            # all_answer_paths.extend(answer_paths_list) # for each query, there are 3 answers
            corrected_answers = list(zip(*answer_paths_list))
        
            # Now extend the main list
            all_answer_paths.extend(corrected_answers)
            all_captions.extend(target_caption)
            # if args.gpu is not None:
            ref_sketches = ref_sketches.cuda(args.gpu, non_blocking=True)
            target_caption = target_caption.cuda(args.gpu, non_blocking=True)
            sketch_features = m.encode_image(ref_sketches)
            caption_features = m.encode_text(target_caption)                            
            id_split = tokenize(["*"])[0][1]            
            query_image_tokens = img2text(sketch_features)          
            ...
            all_caption_features.append(caption_features)
            all_query_sketch_features.append(sketch_features)
            all_composed_features.append(composed_feature)
        print("Computing metrics...")
        # Partial function to compute metrics (Keep gallery features fixed)
        metric_func = partial(get_metrics_aodai, image_features=torch.cat(all_image_features), target_names=all_target_paths, answer_names=all_answer_paths)
        feats = {'composed': torch.cat(all_composed_features)}
        for key, value in feats.items():
            metrics = metric_func(ref_features=value)
            logging.info(
