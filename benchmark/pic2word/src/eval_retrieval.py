...
from eval_utils import evaluate_imgnet_retrieval, evaluate_coco, evaluate_fashion, evaluate_cirr, evaluate_cirr_test, evaluate_aodai
from data import CsvDataset, CustomFolder, ImageList, CsvCOCO, FashionIQ, CIRR, AoDai
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    if args.eval_mode == 'coco':
    ...
    elif args.eval_mode == 'aodai':
        source_dataset = AoDai(transforms=preprocess_val,
                               root="/media02/ltnghia31/HNguyen/BM_ICMR2026/aodai", mode="caps")
        target_dataset = AoDai(transforms=preprocess_val,
                               root="/media02/ltnghia31/HNguyen/BM_ICMR2026/aodai", mode='imgs')
        source_dataloader = DataLoader(
            source_dataset,
            batch_size=args.batch_size,
            shuffle=False,
        ...
        target_dataloader = DataLoader(
            ...
            pin_memory=True,
            drop_last=False)
        print("Evaluating AoDai retrieval...")
        results = evaluate_aodai(model, img2text, args, source_dataloader, target_dataloader)
        with open("../aodai_results.json", "w") as f:
            json.dump(results, f)
